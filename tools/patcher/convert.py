...
convert.py -- converts a FFVII Rebirth costume mod between its two formats.
    ...
    python convert.py "D:\\mods\\SomeMod" --out "D:\\converted"
...
def read_template(source, parts, icon_override=None):
    ...
    meta = dict(
        ...
        icon=icon_override or find_image(source, "icon"),
...
def loose_to_dresscode(source, mods, assume_yes=False, destination=None):
    """
    ...
    meta, outfits = read_template(source, parts, IMAGE_OVERRIDE)
    ...
    output_parent = destination or os.path.dirname(source)
    out_root = os.path.join(output_parent, f"{plugin} (Dresscode)")
    merge_tmp = None
    if exact:
...
def unpack_loose_archive(source, temps, assume_yes, destination=None):
    """
    ...
    if os.path.exists(dest):
        ...
        print("  Use that folder from here on -- dresscode.json is "
              "generated inside it.")
    handled = loose_to_dresscode(
        dest, find_mods(dest), assume_yes, destination=destination
    )
    return 1 if handled is None else handled


def unpack_archive_folder(source, temps, assume_yes, destination=None):
    """
...
def main(argv):
    global IMAGE_OVERRIDE, KEEP_REGISTRATION
    IMAGE_OVERRIDE = None
    out_dir = None
    args = []
    skip_value = False
    for arg in argv:
        ...
        if arg.startswith("--icon="):
            IMAGE_OVERRIDE = os.path.abspath(arg.split("=", 1)[1])
            continue
    ...
    for index, arg in enumerate(argv):
        if arg == "--out" and index + 1 < len(argv):
            ...
            IMAGE_OVERRIDE = os.path.abspath(argv[index + 1])
        ...
        for raw in args:
            ...
            if os.path.isdir(source):
                ...
                if handled is not None:
                    ...
                    continue
                result = unpack_archive_folder(
                    source, temps, assume_yes, destination=out_dir
                )
